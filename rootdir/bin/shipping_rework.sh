...
if [ "$auth" == "1" ];then
	...
	if [ ! -z "$NEW_COUNTRY" ] && [ "$NEW_COUNTRY" != "0" ];then
		...
		logi "[COUNTRY] setenforce: permissive"
		echo "[SHIPPING_REWK][COUNTRY] setenforce: permissive" > /proc/asusevtlog
		sleep 5
		...
		setprop vendor.sys.asus.setenforce 1
		logi "[CID] setenforce: permissive"
		echo "[SHIPPING_REWK][CID] setenforce: permissive" > /proc/asusevtlog
		sleep 5
